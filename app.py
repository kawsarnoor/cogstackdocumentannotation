...
class Label(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True) # primary keys are required by SQLAlchemy  
    label = db.Column(db.String(1000), unique=True, nullable=False) 
...
def changelabel(user):
    ...
    annotatedDocument = AnnotatedDocument.query.filter_by(document_id = document_id, user=user).first()
    ...
    print('changelabel: ', time.time() - start)
...
def getProject(user):
    ...
    return jsonify({'available_labels': available_labels, 'available_label_ids': available_label_ids})
...
def getAnnotatedDocument(user):
    ...
    document_text = Document.query.get(document_id).text
    annotateddocument = AnnotatedDocument.query.filter_by(user=user, document_id=document_id).first()

    if annotateddocument:
        label_id = annotateddocument.label.id
    else:
        label_id = 1

    print('getAnnotatedDocument', time.time() - start)
